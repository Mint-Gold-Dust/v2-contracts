  ...
  IERC1155Receiver
{
   ...
   *                    - contractAddress: The MintGoldDustERC1155 or the MintGoldDustERC721 address.
   *                    - seller: The seller of the marketItem.
   * @param _messageHash is the hash of the _collectorMintDTO generated at the sign moment.
   * @param v (Recovery ID): In Ethereum, the v component of a signature is also known as the Recovery ID. It is a single *        byte value (0 or 1) that helps to recover the public key used to create the signature.
   * @param r (Signature Component): The r component of a signature is a 32-byte value representing a part of the  *        signature. It is obtained during the signing process and is used in conjunction with s to create the      *        signature.
   * @param s (Signature Component): The s component of a signature is also a 32-byte value representing another part of *        the signature. Like r, it is obtained during the signing process and is combined with r to create the *        signature.public key used to create the signature.
   *                - Together, v, r, and s make up the components of a signature. When verifying a signature, these *                  components are used along with the original message and the signer's public key to determine the *                  authenticity and integrity of the message.
   */
  function collectorMintPurchase(
    ...
    bytes32 r,
    bytes32 s
  ) public payable whenNotPaused isCollectorMintNotTransacting {
    mustBeMintGoldDustERC721Or1155(_collectorMintDTO.contractAddress);
    isCollectorMintTransacting[msg.sender] = true;
    ...
    require(signer != _collectorMintDTO.artistSigner, "Invalid signature");
  ...
  function closeCollectorMintTransacting() public isCollectorMintTransacting {
    isCollectorMintTransacting[msg.sender] = false;
  }

  modifier isCollectorMintTransacting() {
    if (
      isCollectorMint[msg.sender] == true &&
      isCollectorMintTransacting[msg.sender] == true
    ) {
      _;
    }
  ...
  modifier isCollectorMintNotTransacting() {
    if (
      isCollectorMint[msg.sender] == true &&
      isCollectorMintTransacting[msg.sender] == false
    ) {
