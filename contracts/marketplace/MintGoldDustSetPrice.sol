...
contract MintGoldDustSetPrice is MintGoldDustMarketplace {
    ...
    function collectorMintPurchase(
        ...
        require(
            _collectorMintDTO.amount > 0 || _amountToBuy > 0,
            "Invalid amount to mint or invalid amount to buy"
        );
        ...
        if (_collectorMintDTO.collaborators.length == 0) {
            _tokenId = callCollectorMint(_mintGoldDustNFT, _collectorMintDTO);
        } else {
            _tokenId = callSplitMint(_mintGoldDustNFT, _collectorMintDTO);
        }

        listCollectorMint(_tokenId, _collectorMintDTO);
    ...
    function callCollectorMint(
        MintGoldDustNFT _mintGoldDustNFT,
        CollectorMintDTO memory _collectorMintDTO
    ) private returns (uint256) {
        uint256 _tokenId = _mintGoldDustNFT.collectorMint(
    ...
    function callSplitMint(
        MintGoldDustNFT _mintGoldDustNFT,
        CollectorMintDTO memory _collectorMintDTO
    ) private returns (uint256) {
        uint256 _tokenId = _mintGoldDustNFT.collectorSplitMint(
            _collectorMintDTO.tokenURI,
            _collectorMintDTO.royalty,
    ...
    function generateEIP712Hash(
        ...
        require(hashBytes32 == _eip712HashOffChain, "Invalid hash");
    }
