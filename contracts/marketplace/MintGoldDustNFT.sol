  ...
  ReentrancyGuardUpgradeable
{
  ...
  function collectorMint(
    ...
    bytes calldata _memoir,
    uint256 _collectorMintId
  ) external onlySetPrice whenNotPaused returns (uint256) {
    uint256 newTokenId = executeMintFlow(
      _tokenURI,
      _royaltyPercent,
      _amountToMint,
  ...
  function executeSplitMintFlow(
    ...
    address[] calldata _newOwners,
    uint256[] calldata _ownersPercentage
  ) private arrayLengthCheck(_newOwners, _ownersPercentage) {
    ...
    for (uint256 i = 0; i < _newOwners.length; i++) {
      ...
      require(_ownersPercentage[i] > 0, "Owner percentage must be > 0");
    ...
    require(_ownersPercentage[ownersCount] > 0, "Owner percentage must be > 0");
