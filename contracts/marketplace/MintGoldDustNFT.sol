...
abstract contract MintGoldDustNFT is Initializable, PausableUpgradeable {
  ...
  MintGoldDustMarketplace private mintGoldDustMarketplace;
   ...
   * @param _memoir the memoir for this token.
   */
  function mintNft(
    string calldata _tokenURI,
    uint256 _royaltyPercent,
  ...
  function splitMint(
    ...
    uint256 _amount,
    bytes calldata _memoir
  ) external whenNotPaused isApproved(msg.sender) returns (uint256) {
  ...
  function executeSplitMintFlow(
    ...
    tokenIdCollaboratorsPercentage[_tokenId][ownersCount] = _ownersPercentage[
      ownersCount
    ];
  ...
  modifier isApproved(address _msgSender) {
    if (!mintGoldDustCompany.isArtistApproved(_msgSender)) {
      revert Unauthorized();
    }
    _;
  }
  ...
  modifier isCollectorMintTransacting() {
    if (
      mintGoldDustCompany.isCollectorMint(msg.sender) == true &&
