  ...
  PausableUpgradeable
{
  ...
  /**
   * This struct consists of the following fields:
   *    - auctionId: is a counter used to identify each auction.
   ...
   *    - cancelled: a boolean that indicates if the auction was cancelled or not.
   ...
   *                    - collectorMintId: the id created off chain to track the collector mint flow off chain and on *                      chain.
   *                        @dev this id will be triggered at the real mint moment. So in the backend will be possible *                             to track the flow.
   */
  ...
  function primarySale(
    ...
    idMarketItemsByContractByOwner[_saleDTO.contractAddress][_saleDTO.tokenId][
      _saleDTO.seller
    ].sold = true;
    idMarketItemsByContractByOwner[_saleDTO.contractAddress][_saleDTO.tokenId][
      _saleDTO.seller
    ...
    fee =
      ...
      (100 * 10 ** 18);
    collFee = (_value * mintGoldDustCompany.collectorFee()) / (100 * 10 ** 18);
  ...
  function splittedSale(
    ...
    for (uint256 i = 1; i < _tokenIdCollaboratorsQuantity; i++) {
      balanceSplitPart =
        ...
        (100 * 10 ** 18);
      payable(_mintGoldDustNFT.tokenCollaborators(_saleDTO.tokenId, i - 1))
   ...
   *                    - seller: The seller of the marketItem.
   */
  function purchaseNft(SaleDTO memory _saleDTO) external payable whenNotPaused {
  ...
  /**
   * @notice that this function is used to generate a hash for a CollectorMintDTO struct.
   * @param _collectorMintDTO a CollectorMintDTO struct that will be used to generate a hash.
   */
  ...
  /**
   * @notice that this function is used to check if the hash received is the same of the hash generated.
   * @param _collectorMintDTO a CollectorMintDTO struct that will be used to generate a hash.
   * @param receivedHash an already generated hash that will be compared with the generated one.
   */
  function verifyHash(
    CollectorMintDTO memory _collectorMintDTO,
  ...
  modifier isNotListed(
    ...
    if (
      !idMarketItemsByContractByOwner[_contractAddress][_tokenId][_seller].sold
    ) {
