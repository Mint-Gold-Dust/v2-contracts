    ...
    ReentrancyGuardUpgradeable
{
    ...
    function uniqueOwnerPrimarySale(
        ...
        (bool sent, bytes memory data) = payable(_marketItem.seller).call{
            value: _balance
        }("");
    ...
    function updateIdMarketItemsByContractByOwnerMapping(
        ...
        //   Avoid updating storage twice - decrement amount and delete entry in one branch
        // Compare against full amount first to potentially save a SSTORE by deleting the entry early
        SaleDTO memory _saleDTO
    ) private {
        MarketItem storage item = idMarketItemsByContractByOwner[
            _saleDTO.contractAddress
        ...
        if (item.tokenAmount == _saleDTO.amount) {
            // If selling the full amount, delete the mapping entry
            delete idMarketItemsByContractByOwner[_saleDTO.contractAddress][
                _saleDTO.tokenId
            ][_saleDTO.seller];
        } else {
            ...
            unchecked {
                item.tokenAmount -= _saleDTO.amount;
            }
        }
    }
