...
function addCollectorFee(bigish: BigNumber) {
  return bigish.add(bigish.mul("3").div("100"));
}

describe("RefactorPrimarySaleStorage Upgrading Tests", function () {
  /**
  ...
  async function testSetupFixture() {
    ...
    await mgd721
      .connect(artist1)
      .mintNft(FAKE_URI, FAKE_ROYALTY, 1, FAKE_MEMOIR);
    await mgd721
      .connect(artist1)
      .splitMint(
        FAKE_URI,
    ...
    await mgdAuction
      .connect(artist2)
      .list(tokenId1155_3, 10, mgd1155.address, LOW_PRICE);
    await mgdAuction
      .connect(artist2)
      .list(tokenId1155_4, 10, mgd1155.address, LOW_PRICE);
    ...
    await mgdSetPrice.connect(collector2).purchaseNft(
      ...
      {
        value: addCollectorFee(LOW_PRICE.mul("10")),
      }
    );
    ...
    await stopImpersonating(ADDR_collector1);
  ...
  async function testUpgradingFixture() {
    const { mgdOwnerSigner } = await loadFixture(testSetupFixture);

    // Deal company owner signer some more ETH
    await dealEth(mgdOwnerSigner.address, 10);
    ...
    // Perform upgrades
    await impersonate(mgdOwnerSigner.address);
    const mgd721Up = await upgrades.upgradeProxy(ADDR_mgdNft721, mgd721Factory);
    const mgd1155Up = await upgrades.upgradeProxy(
      ADDR_mgdNft1155,
      mgd1155Factory
    );
    const mgdSetPriceUp = await upgrades.upgradeProxy(
      ADDR_mgdSetPrice,
      mgdSetPriceFactory
    );
    const mgdAuctionUp = await upgrades.upgradeProxy(
      ADDR_mgdAuction,
      mgdAuctionFactory
    );
  ...
  it("should check collector 1 owns corresponding NFTs", async () => {
    const { collector1, mgd721, mgd1155, tokenId721_1, tokenId1155_3 } =
      await loadFixture(testSetupFixture);
    ...
    expect(collector1Balance1155_tokenId_3).to.equal("10");
  });
});
