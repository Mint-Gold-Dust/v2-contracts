...
describe("\nMintGoldDustMaretplaceAuction.sol + MintGoldDustERC1155.sol Smart Contracts \n************___************\n \nHere we'll have the tests related of an auction flow for the MintGoldDustERC1155 token. \n", function () {
  ...
  describe("\n \n **************__************** PLACE A BID GENERAL UNHAPPY PATHS **************__**************", function () {
    ...
    let quantityToMint = 10;
    let quantityToList = 5;
    let realPrice = price * quantityToList;
    ...
    it("Should revert with an AuctionMustBeEnded() error when some user tries to bid in a timed auction that have ended already.", async function () {
      ...
      await expect(
        ...
        .withArgs(1, mintGoldDustERC1155.address, 1);
    });

    it("Should revert with an AuctionCreatorCannotBid() error if the auction creator (mintGoldDustERC1155 Owner) tries to place a bid.", async function () {
      await mintGoldDustMarketplaceAuction
        .connect(addr1)
        .list(1, quantityToList, mintGoldDustERC1155.address, toWei(price));
      await expect(
  ...
  describe("\n \n ****************_**************** PLACE A BID HAPPY PATHS ****************_****************\n", function () {
    ...
    describe("\n\t------------------ AUCTION WITH A RESERVE PRICE ------------------\n", () => {
      let realPrice = price + (price * 3) / 100;
  ...
  describe("\n \n ****************_**************** END AUCTION UNHAPPY PATHS ****************_****************\n", function () {
    ...
    const quantityToPay = quantityToList * price;
    ...
    it("Should revert with Unauthorized error if the end auction function is called and the tokenId was not listed on MintGoldDustMarketplaceAuction.", async () => {
  ...
  describe("\n \n ****************_**************** END AUCTION FOR PRIMARY SALE ****************_****************\n", function () {
    ...
    let priceToBuy = price * quantityToList;
    ...
    it("Should:\n \t - Simulate a primary sale that transfer a mintGoldDustERC1155 to the buyer;\n \t - Verify if the item changed status for sale; \n \t - Verify if the hasCollaborator flag is false; \n \t - Check if the isERC721 attribute is true;\n \t -  And also the isAuction attribute must be true;\n \t - Verify if the seller balance increases;\n \t - Verify if the marketplace's owner receives the fee;\n \t - Verify if the isSecondarySale attribute was set to true;\n \t - Verify if the buyer balance was deacresed exactly the gas fee + the token price;", async function () {
      ...
      expect(receipt.events[2].args.tokenId).to.be.equal(1);
      expect(receipt.events[2].args.contractAddress).to.be.equal(
        mintGoldDustERC1155.address
      );
      expect(receipt.events[2].args.seller).to.be.equal(addr1.address);
      expect(receipt.events[2].args.endTime).to.be.equal(
        (await receipt.events[0].getBlock()).timestamp
      );
      expect(receipt.events[2].args.auctionId).to.be.equal(1);
  ...
  describe("------------------ END AUCTION NFT FOR SECONDARY MARKET ------------------", function () {
    ...
    beforeEach(async () => {
      ...
      // Artist approve gdMarketPlace marketplace to exchange its mintGoldDustERC1155
      await mintGoldDustERC1155
        .connect(addr1)
        .setApprovalForAll(mintGoldDustMarketplaceAuction.address, true);
    ...
    it("Should simulate a secondary sale that transfer a mintGoldDustERC1155 to the buyer, verify if the item changed status for sale, verify if the seller balance increases and also if the marketplace's owner receives the fee and verify if the artist creator have received the royalty. Verify if the hasCollaborator flag is false, the isERC721 attribute is true and if the isAuction attribute is true.", async function () {
      ...
      // get the mintGoldDustERC1155's artist creator balance
      const provider = ethers.provider;
      ...
      expect(receipt.events[1].args.tokenId).to.be.equal(1);
      expect(receipt.events[1].args.seller).to.be.equal(addr2.address);
      ...
      expect(receipt.events[2].args.seller).to.be.equal(addr2.address);
      expect(receipt.events[2].args.endTime).to.be.equal(
        (await receipt.events[0].getBlock()).timestamp
      );
      expect(receipt.events[2].args.auctionId).to.be.equal(2);
