...
describe("MintGoldDustSetPrice.sol Smart Contract \n___________________________________________________\n \nThis smart contract is responsible by all functionalities related with the fixed price market. \n Here goes the tests related with the MintGoldDustSetPrice market and the MintGoldDustERC721 tokens. \n\n", function () {
  ...
  describe("\n--------------- Test related with the burn NFT functionality ---------------\n", function () {
    it("should allow token owner to burn token", async function () {
      // Setup: Whitelist and Mint new tokens with tokenId = 1 for addr1
      ...
      await mintGoldDustERC1155
        .connect(addr1)
        .mintNft("AnotherURI", toWei(5), initialAmount, bytesMemoir);

      // Initially check ownership
      const initialBalance = await mintGoldDustERC1155.balanceOf(
        addr1.address,
        1
      );
      expect(initialBalance).to.equal(initialAmount);

      const burnAmount = 5; // Set this to the amount you want to burn
      // Burn the token
      await expect(
        mintGoldDustERC1155.connect(addr1).burn(addr1.address, 1, burnAmount)
      )
        .to.emit(mintGoldDustERC1155, "TransferSingle") // Replace "TransferSingle" with your actual event if different
        .withArgs(
          addr1.address,
          addr1.address,
      ...
      // Check that the token is burned (balance should be reduced)
      const finalBalance = await mintGoldDustERC1155.balanceOf(
        addr1.address,
        1
      );
    ...
    it("should allow contract owner to burn any token", async function () {
      // Setup: Whitelist and Mint new tokens with tokenId = 2 for addr1
      await mgdCompany.connect(deployer).whitelist(addr1.address, true);
      const initialAmount = 10;
      ...
      // Initially check ownership (balance)
      const initialBalance = await mintGoldDustERC1155.balanceOf(
        addr1.address,
        1
      );
      ...
      await expect(
        mintGoldDustERC1155.connect(deployer).burn(addr1.address, 1, burnAmount)
      )
        .to.emit(mintGoldDustERC1155, "TransferSingle") // Replace "TransferSingle" with your actual event if different
        .withArgs(
          deployer.address,
    ...
    it("should allow approved address to burn token", async function () {
      ...
      // Approve addr2 for all tokens
      await mintGoldDustERC1155
        .connect(addr1)
        .setApprovalForAll(addr2.address, true);

      // Check approval
      const isApproved = await mintGoldDustERC1155.isApprovedForAll(
        addr1.address,
        addr2.address
      );
      ...
      await expect(
        mintGoldDustERC1155.connect(addr2).burn(addr1.address, 1, burnAmount)
      )
        .to.emit(mintGoldDustERC1155, "TransferSingle") // Replace "TransferSingle" with your actual event if different
        .withArgs(
          addr2.address,
    ...
    it("should not allow non-owner to burn token", async function () {
      ...
      await expect(
        mintGoldDustERC1155.connect(addr3).burn(addr1.address, 1, 1)
    ...
    it("should not allow non-contract owner to burn token they don't own", async function () {
      ...
      // Attempt to burn the token using addr2 (who doesn't own the token) should fail
      await expect(
        mintGoldDustERC1155.connect(addr2).burn(addr1.address, 1, 1)
      ).to.be.revertedWith("Only owner or approved"); // Replace with your actual error message
    });

    it("should not allow burning a non-existent token", async function () {
      // Initially check that addr1 does not have a token with tokenId = 99
      const initialBalance = await mintGoldDustERC1155.balanceOf(
        addr1.address,
        99
      );
      ...
      await expect(
        mintGoldDustERC1155.connect(addr1).burn(addr1.address, 99, 1)
    ...
    it("should not allow transferring a burned token", async function () {
      ...
      const initialAmount = 10;
      const encoder = new TextEncoder();
      const bytesMemoir = encoder.encode("Soon to be burned");

      await mintGoldDustERC1155
        .connect(addr1)
        .mintNft("SoonToBeBurnedURI", toWei(5), initialAmount, bytesMemoir);

      // Initially check balance
      const initialBalance = await mintGoldDustERC1155.balanceOf(
        addr1.address,
        1
      );
      ...
      // Burn all the tokens of tokenId = 1
      await mintGoldDustERC1155
        .connect(addr1)
        .burn(addr1.address, 1, initialAmount);

      // Attempt to transfer the burned token should fail
      await expect(
        mintGoldDustERC1155.connect(addr1).safeTransferFrom(
          addr1.address,
          addr2.address,
          1,
          1, // attempting to transfer 1 token
          []
        )
      ).to.be.revertedWith("ERC1155: insufficient balance for transfer"); // Replace with your actual error message
    });
    ...
    it("should not allow re-burning an already burned token", async function () {
      ...
      const initialAmount = 10;
      const encoder = new TextEncoder();
      const bytesMemoir = encoder.encode("Burn me once");

      await mintGoldDustERC1155
        .connect(addr1)
        .mintNft("BurnMeOnceURI", toWei(5), initialAmount, bytesMemoir);
      ...
      await expect(
        mintGoldDustERC1155.connect(addr1).burn(addr1.address, 1, 1)
      ).to.be.revertedWith("Insufficient balance to burn"); // Replace with your actual error message
    });
