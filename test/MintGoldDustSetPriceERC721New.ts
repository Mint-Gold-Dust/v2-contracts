...
describe("\nMGDSetPrice.sol Smart Contract \n___________________________________________________\n \nThis smart contract is responsible by all functionalities related with the fixed price market. \n Here goes the tests related with the MintGoldDustSetPrice market and the MintGoldDustERC721 tokens. \n\n", function () {
  ...
  describe("\n--------------- Tests related witn the list NFT functionality ---------------\n", function () {
    ...
    beforeEach(async () => {
      ...
      // addr1 mints a nft
      await mintGoldDustERC721.connect(addr1).mintNft(URI, toWei(5), 10);
    ...
    it("Should revert with a MintGoldDustMustBeERC721OrERC1155 error if the contract address trying to list is neither a ERC721 nor a ERC1155.", async function () {
      await expect(
        mgdSetPrice.connect(addr1).list(1, 1, mgdCompany.address, toWei(price))
      ...
      await expect(
        mgdSetPrice.connect(addr1).list(1, 1, addr1.address, toWei(price))
      ).to.be.revertedWithCustomError(
        mgdSetPrice,
        "MintGoldDustMustBeERC721OrERC1155"
      );
    ...
    it("Should track a listing of a MintGoldDustERC1155", async function () {
      ...
      await expect(
        mgdSetPrice
          .connect(addr1)
          .list(1, 5, mintGoldDustERC721.address, toWei(price))
      )
        .to.emit(mgdSetPrice, "MintGoldDustNftListedToSetPrice")
    ...
    it("Should secondary sale keep false if an artist list, delist and list again an item.", async function () {
      ...
      await mgdSetPrice.connect(addr1).delistNft(
      {
        tokenId:1, 
      ...
      expect(
        (
          await mgdSetPrice
            .connect(addr1)
            .idMarketItemsByContractByOwner(mintGoldDustERC721.address, 1, addr1.address)
        ).isSecondarySale
      ).to.be.equal(false);
    });
    ...
    it("Should revert the transaction if an artist tries to list its nft with price less than or equal zero.", async function () {
      await expect(
        mgdSetPrice.connect(addr1).list(1, 0, mintGoldDustERC721.address, 0)
      ).to.be.revertedWithCustomError(
        mgdSetPrice,
    ...
    it("Should revert the transaction with a MGDMarketplaceUnauthorized error if an artist is not the owner of the token and try to list on the gold dust marketplace.", async function () {
      await expect(
        mgdSetPrice
          .connect(addr2)
          .list(1, 1, mintGoldDustERC721.address, toWei(price))
      ).to.revertedWithCustomError(mgdSetPrice, "MintGoldDustAddressUnauthorized");
    });
  });
...
//       await mgdCompany.connect(deployer).whitelist(addr1.address, true);
//       // addr1 mints a nft
//       await mintGoldDustERC721.connect(addr1).mintNft(URI, toWei(5), 1);
//       // Artist approve gdMarketPlace marketplace to exchange its NFT
//       await mintGoldDustERC721
//         .connect(addr1)
//         .setApprovalForAll(mgdSetPrice.address, true);
...
//       await mgdSetPrice
//         .connect(addr1)
//         .list(1, 1, mintGoldDustERC721.address, toWei(primaryPrice));
//     });
...
//         mgdSetPrice
//           .connect(addr1)
//           .updateListedNft(1, toWei(newPrice), mgdCompany.address, addr1.address)
//       ).to.be.revertedWithCustomError(
//         mgdSetPrice,
//         "MintGoldDustMustBeERC721OrERC1155"
//       );
...
//       let marketItem = await mgdSetPrice.idMarketItemsByContractByOwner(
//         mintGoldDustERC721.address,
//         1
//       );
...
//         await mgdSetPrice
//           .connect(addr1)
//           .updateListedNft(1, toWei(newPrice), mintGoldDustERC721.address)
//       )
...
//       marketItem = await mgdSetPrice.idMarketItemsByContractByOwner(
//         mintGoldDustERC721.address,
//         1
//       );
...
//         mgdSetPrice
//           .connect(addr1)
//           .updateListedNft(1, toWei(0), mintGoldDustERC721.address)
//       ).to.be.revertedWithCustomError(
//         mgdSetPrice,
...
//       await mgdSetPrice
//         .connect(addr1)
//         .list(1, toWei(primaryPrice), 5, mintGoldDustERC721.address);
//     });
...
//         await mgdSetPrice
//           .connect(addr1)
//           .delistNft(1, mintGoldDustERC721.address)
//       )
...
//       // addr2 relist a purchased NFT
//       await expect(
//         mgdSetPrice.connect(addr2).delistNft(1, mintGoldDustERC721.address)
//       ).to.be.revertedWithCustomError(
//         mgdSetPrice,
//         "MGDMarketplaceUnauthorized"
//       );
...
//       await mgdSetPrice
//         .connect(addr1)
//         .list(1, toWei(price), 5, mintGoldDustERC721.address);
...
//         await mgdSetPrice
//           .connect(addr2)
//           .purchaseNft(1, 3, mintGoldDustERC721.address, {
//             value: toWei(price),
//           })
//       )
...
//     it("Should revert with MGDMarketplaceItemIsNotListed error if the user tries to buy a MintGoldDustERC721 that was already sold.", async () => {
//       await mgdSetPrice
//         .connect(addr2)
//         .purchaseNft(1, 5, mintGoldDustERC721.address, {
//           value: toWei(price),
//         });
//       await expect(
//         mgdSetPrice
