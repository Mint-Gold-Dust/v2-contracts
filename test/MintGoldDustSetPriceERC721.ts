...
describe("\nMGDSetPrice.sol Smart Contract \n___________________________________________________\n \nThis smart contract is responsible by all functionalities related with the fixed price market. \n Here goes the tests related with the MintGoldDustSetPrice market and the MintGoldDustERC721 tokens. \n\n", function () {
  ...
  describe("\n--------------- Tests related witn the list NFT functionality ---------------\n", function () {
    ...
    it("Should revert with a MustBeERC721OrERC1155 error if the contract address trying to list is neither a ERC721 nor a ERC1155.", async function () {
      await expect(
        mgdSetPrice.connect(addr1).list(1, toWei(price), 1, mgdCompany.address)
      ).to.be.revertedWithCustomError(mgdSetPrice, "MustBeERC721OrERC1155");

      await expect(
        mgdSetPrice.connect(addr1).list(1, toWei(price), 1, addr1.address)
      ).to.be.revertedWithCustomError(mgdSetPrice, "MustBeERC721OrERC1155");
    });
    ...
    it("Should secondary sale keep false if an artist list, delist and list again an item.", async function () {
      ...
      await mgdSetPrice.connect(addr1).delistNft(1, mintGoldDustERC721.address);
      await mgdSetPrice
        .connect(addr1)
        .list(1, toWei(price), 1, mintGoldDustERC721.address);
    ...
    it("Should revert the transaction if an artist tries to list its nft with price less than or equal zero.", async function () {
      await expect(
        mgdSetPrice.connect(addr1).list(1, 0, 1, mintGoldDustERC721.address)
  ...
  describe("\n--------------- Tests related with the update a listed NFT functionality ---------------\n", function () {
    ...
    beforeEach(async () => {
      ...
      await mgdSetPrice
        .connect(addr1)
        .list(1, toWei(primaryPrice), 1, mintGoldDustERC721.address);
    });
    ...
    it("Should revert with a MustBeERC721OrERC1155 error if the contract address trying to list is neither a ERC721 nor a ERC1155.", async function () {
      await expect(
        mgdSetPrice
          .connect(addr1)
          .updateListedNft(1, toWei(newPrice), mgdCompany.address)
      ).to.be.revertedWithCustomError(mgdSetPrice, "MustBeERC721OrERC1155");

      await expect(
        mgdSetPrice
          .connect(addr1)
          .updateListedNft(1, toWei(newPrice), addr1.address)
      ).to.be.revertedWithCustomError(mgdSetPrice, "MustBeERC721OrERC1155");
    });
    ...
    it("Should track if a listed item was correctly updated and emit the NftListedItemUpdated event. We should remember that in this moment the owner of the NFT is the marketplace.", async function () {
      ...
      let marketItem = await mgdSetPrice.idMarketItemsByContract(
        mintGoldDustERC721.address,
        1
      );
      ...
      marketItem = await mgdSetPrice.idMarketItemsByContract(
        mintGoldDustERC721.address,
        1
      );
