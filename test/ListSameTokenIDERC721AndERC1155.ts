...
describe("\nnMintGoldDustMaretplace.sol + MintGoldDustMaretplaceAuction.sol + MintGoldDustERC1155.sol + MintGoldDustERC721.sol Smart Contracts \n************___************\n \nHere we'll have the tests that simulate a listing of the same token id but for the different MintGoldDustNFT contracts (1155 abd 721). \n", function () {
  ...
  describe("\n****************_**************** Tests related with listing a MintGoldDustERC1155 and a MintGoldDustERC721 for auction ****************_****************\n", function () {
    ...
    it("Should be possible the same artist list the same tokenId but for diffent contracts (MintGoldDustERC1155 and MintGoldDustERC721).", async function () {
      ...
      expect(receipt2.events[1].event).to.equal("ItemListedToAuction");
      // eventSignature: 'ItemListedToAuction(uint256,address,address,uint256,uint256,uint256)'
      expect(receipt2.events[1].eventSignature).to.equal(
        "ItemListedToAuction(uint256,address,address,uint256,uint256,uint256)"
      );
      expect(receipt2.events[1].args[0]).to.equal(1);
      expect(receipt2.events[1].args[1]).to.equal(mintGoldDustERC721.address);
      expect(receipt2.events[1].args[2]).to.equal(addr1.address);
      expect(receipt2.events[1].args[3]).to.equal(toWei(price));
      expect(receipt2.events[1].args[4]).to.equal(
        (await receipt2.events[0].getBlock()).timestamp
      );
      expect(receipt2.events[1].args[5]).to.equal(2);
    });
  });
});
